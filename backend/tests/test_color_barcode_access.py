...
async def test_color_barcode_management_allows_authorized_departments(
...
@pytest.mark.anyio
@pytest.mark.parametrize("department_code", ["运营部", "财务部", "美工部"])
async def test_color_barcode_management_rejects_other_departments(
    ...
    monkeypatch.setattr(
        auth_middleware,
        "get_current_user_from_request",
        lambda _: {"role_code": "department_user", "department_code": department_code},
    )
