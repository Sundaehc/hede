    ...
    <div className="flex h-full flex-col">
      ...
      <Tabs defaultValue="base" className="min-h-0 flex-1">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          ...
          <TabsContent value="stock" className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><SummaryMetric icon={Boxes} label="在仓合计" value={item.stock_total} /><SummaryMetric icon={Boxes} label="在途合计" value={item.in_transit_total} /><SummaryMetric icon={Boxes} label="整体库存合计" value={item.inventory_total} /><SummaryMetric icon={Boxes} label="缺货合计" value={metric(item, "shortage_total")} /><SummaryMetric icon={Boxes} label="回单" value={metric(item, "return_qty")} /><SummaryMetric icon={Boxes} label="补单合计" value={metric(item, "replenishment_total")} /><SummaryMetric icon={Boxes} label="补单后库存" value={metric(item, "post_replenishment_stock")} /><SummaryMetric icon={Boxes} label="补单后合计" value={metric(item, "post_replenishment_total")} /><SummaryMetric icon={CalendarDays} label="补单后周转天数" value={metric(item, "post_replenishment_turnover_days")} /></div><SizeGrid title="在仓库存" values={item.stock_by_size} sizes={data.size_columns} /><SizeGrid title="在途库存" values={item.in_transit_by_size} sizes={data.size_columns} /><SizeGrid title="库存合计" values={item.inventory_by_size} sizes={data.size_columns} /><SizeGrid title="缺货库存" values={item.shortage_by_size} sizes={data.size_columns} /><SizeGrid title="销售明细" values={item.sales_by_size} sizes={data.size_columns} /><SizeGrid title="补单明细" values={item.replenishment_by_size} sizes={data.size_columns} /><SizeGrid title="补单后尺码" values={item.post_replenishment_by_size} sizes={data.size_columns} /><ManualReplenishmentForm item={item} sizes={data.size_columns} canEdit={canEdit} onSave={onSave} /></TabsContent>
        </div>
      </Tabs>
    </div>
  </div>
