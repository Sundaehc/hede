...
type ReplenishmentDraft = {
  expected_replenishment_stock: string
}
...
function replenishmentDraft(item: ProductGoodsItem): ReplenishmentDraft {
  return {
    expected_replenishment_stock: textNumber(item.metrics?.expected_replenishment_stock),
  }
}
...
function ManualReplenishmentForm({ item, sizes, canEdit, onSave }: { item: ProductGoodsItem; sizes: string[]; canEdit: boolean; onSave: DrawerProps["onSave"] }) {
  ...
  function updateValue(value: string) {
    setFields((current) => ({ ...current, expected_replenishment_stock: value }))
  }

  if (!canEdit) return <DetailSection title="补单信息"><div className="grid gap-2 sm:grid-cols-2"><DetailField label="预计补单库存" value={metric(item, "expected_replenishment_stock")} /><DetailField label="补单后总库存" value={metric(item, "post_replenishment_total")} /><DetailField label="补单后周转天数" value={metric(item, "post_replenishment_turnover_days")} /></div><SizeGrid title="补单后库存明细" values={item.post_replenishment_by_size} sizes={sizes} /></DetailSection>

  return <form className="space-y-4 border-t border-border pt-5" onSubmit={(event) => {
    event.preventDefault()
    setSaving(true)
    void onSave(item, {
      expected_replenishment_stock: numberOrNull(fields.expected_replenishment_stock),
    }).finally(() => setSaving(false))
  }}>
    <DetailSection title="预计补单库存"><label className="grid max-w-xs gap-1.5 text-sm font-medium">回单<Input type="number" min="0" step="1" inputMode="numeric" value={fields.expected_replenishment_stock} onChange={(event) => updateValue(event.target.value)} disabled={saving} /></label></DetailSection>
    <div className="grid gap-3 sm:grid-cols-2"><DetailField label="补单后总库存" value={metric(item, "post_replenishment_total")} /><DetailField label="补单后周转天数" value={metric(item, "post_replenishment_turnover_days")} /></div>
    <SizeGrid title="近14天销售明细" values={item.sales_by_size} sizes={sizes} />
    <SizeGrid title="补单后库存明细" values={item.post_replenishment_by_size} sizes={sizes} />
    ...
    <div className="flex h-full flex-col">
      ...
      <Tabs defaultValue="base" className="min-h-0 flex-1">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          ...
          <TabsContent value="stock" className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><SummaryMetric icon={Boxes} label="在仓合计" value={item.stock_total} /><SummaryMetric icon={Boxes} label="在途合计" value={item.in_transit_total} /><SummaryMetric icon={Boxes} label="整体库存合计" value={item.inventory_total} /><SummaryMetric icon={Boxes} label="缺货合计" value={metric(item, "shortage_total")} /><SummaryMetric icon={Boxes} label="退货数量" value={metric(item, "return_qty")} /><SummaryMetric icon={Boxes} label="预计补单库存" value={metric(item, "expected_replenishment_stock")} /><SummaryMetric icon={Boxes} label="补单后库存" value={metric(item, "post_replenishment_stock")} /><SummaryMetric icon={Boxes} label="补单后合计" value={metric(item, "post_replenishment_total")} /><SummaryMetric icon={CalendarDays} label="补单后周转天数" value={metric(item, "post_replenishment_turnover_days")} /></div><SizeGrid title="在仓库存" values={item.stock_by_size} sizes={data.size_columns} /><SizeGrid title="在途库存" values={item.in_transit_by_size} sizes={data.size_columns} /><SizeGrid title="库存合计" values={item.inventory_by_size} sizes={data.size_columns} /><SizeGrid title="缺货库存" values={item.shortage_by_size} sizes={data.size_columns} /><SizeGrid title="近14天销售明细" values={item.sales_by_size} sizes={data.size_columns} /><SizeGrid title="补单明细" values={item.replenishment_by_size} sizes={data.size_columns} /><SizeGrid title="补单后库存明细" values={item.post_replenishment_by_size} sizes={data.size_columns} /><ManualReplenishmentForm item={item} sizes={data.size_columns} canEdit={canEdit} onSave={onSave} /></TabsContent>
        </div>
      </Tabs>
    </div>
  </div>
