...
import { ChevronDown, ChevronRight, Edit, GripVertical, History, Plus, Search, Trash2, X } from "lucide-react"
...
export function GeneralCustomerShopsTab({ standalone = false }: GeneralCustomerShopsTabProps) {
  ...
  const [collapsedShopIds, setCollapsedShopIds] = useState<Set<number>>(() => new Set())
  const [ledgerTargetName, setLedgerTargetName] = useState<string | null>(null)
  ...
  const openUnit = (shop: GeneralCustomerShopItem, item?: GeneralCustomerUnitItem) => {
    setCollapsedShopIds((current) => {
      if (!current.has(shop.id)) return current
  ...
  return (
    <div className={standalone ? "app-page" : "surface-panel p-4"}>
          ...
          <section className="surface-panel p-4">
            ...
            <div className="mt-3 space-y-3">
              ...
              {visibleShops.map((shop) => {
                const shopUnits = units.filter((unit) => unit.shop_id === shop.id)
                const isShopUnitsCollapsed = collapsedShopIds.has(shop.id)
                return (
      ...
      <CounterpartyLedgerDialog open={ledgerTargetName !== null} counterpartyType="customer" name={ledgerTargetName || ""} onOpenChange={(open) => { if (!open) setLedgerTargetName(null) }} />
